...
object AvfDiagnostics {
     ...
     * YourXDemon drives (raw kernel + initrd). A vendor build that ships AVF for
    ...
    fun runSmokeTest(context: Context): String {
        ...
        if (AvfCapabilities.choose(pre.capabilitiesRaw) is AvfCapabilities.ProtectedVmChoice.Unsupported) {
            return "not applicable on this device: the hypervisor only supports protected VMs " +
                "(caps=${pre.capabilitiesDecoded}), and YourXDemon's custom Linux kernel can run only as a " +
                "non-protected VM. This is expected, not a failure: YourXDemon automatically uses the QEMU " +
                "backend here."
        }
        ...
        return try {
            ...
            if (cause is UnsupportedOperationException &&
                ...
                "not applicable on this device: AVF rejected a non-protected VM " +
                    "(${cause.message}). YourXDemon's custom Linux kernel can run only as a non-protected " +
                    "VM. This is expected, not a failure: YourXDemon automatically uses the QEMU backend here."
            } else {
