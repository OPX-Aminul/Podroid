...
fun X11Screen(
    ...
    LaunchedEffect(s.rotationLock) {
        activity?.requestedOrientation = when (s.rotationLock) {
            com.opx.yourxdemon.x11.RotationLock.LANDSCAPE -> ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE
            com.opx.yourxdemon.x11.RotationLock.PORTRAIT  -> ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT
            com.opx.yourxdemon.x11.RotationLock.AUTO      -> ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED
        }
    }
    ...
    // YourXDemonService is partial/CPU-only; this is the screen-on counterpart.
    DisposableEffect(Unit) {
