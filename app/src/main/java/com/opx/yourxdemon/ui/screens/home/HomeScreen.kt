...
import com.opx.yourxdemon.engine.avf.AvfFailureGuidance
...
import com.opx.yourxdemon.ui.components.YourXDemonStatus
import com.opx.yourxdemon.ui.components.YourXDemonStatusColors
        ...
        AdaptiveContainer(
            ...
            if (isCompactHeight) {
                ...
                Row(
                    modifier = Modifier
                        .fillMaxSize()
                        .padding(horizontal = YourXDemonTokens.Spacing.XL2, vertical = YourXDemonTokens.Spacing.LG),
                    verticalAlignment = Alignment.CenterVertically,
                ) {
                    Column(
                        modifier = Modifier
                            .weight(1f)
                            .padding(end = YourXDemonTokens.Spacing.XL2)
                    ...
                    Column(
                        ...
                        HomeActionButtons(
                            ...
                            vmState = vmState,
                            onStart = { viewModel.startYourXDemon() },
...
private fun AvfHintBanner(onDismiss: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .padding(bottom = YourXDemonTokens.Spacing.MD),
        colors = CardDefaults.cardColors(
            containerColor = MaterialTheme.colorScheme.surfaceVariant,
        ),
    ) {
        Column(
            modifier = Modifier.padding(YourXDemonTokens.Spacing.MD),
            verticalArrangement = Arrangement.spacedBy(YourXDemonTokens.Spacing.SM),
        ) {
            Text(
...
private fun HomeStatusBlock(
    ...
    YourXDemonSectionLabel(stringResource(R.string.vm_status))
    Text(
        text = when {
    ...
    Row(
        ...
        if (isStopping) {
            ...
            val (dot, label) = when {
                isRunning  -> YourXDemonStatusColors.Running  to (uptimeLabel ?: stringResource(R.string.up))
                isStarting -> YourXDemonStatusColors.Starting to bootStage.ifEmpty { stringResource(R.string.status_starting) }
                else       -> YourXDemonStatusColors.Stopped  to stringResource(R.string.status_idle)
            }
            YourXDemonStatus(label = label, dotColor = dot)
        }
        Text(
    ...
    if (!isRunning && !isStarting && !isStopping) {
        Spacer(Modifier.height(YourXDemonTokens.Spacing.MD))
        YourXDemonSectionLabel(stringResource(R.string.home_containers_created))
        Text(
    ...
    if (vmState is VmState.Error) {
        Spacer(Modifier.height(YourXDemonTokens.Spacing.MD))
        YourXDemonSectionLabel(stringResource(R.string.error_title))
        Text(
...
private fun HomeDataSection(
    ...
    if (isRunning) {
        ...
        YourXDemonSectionLabel(stringResource(R.string.last_session))
...
private fun HomeActionButtons(
    ...
    if (isStopping) {
        ...
        YourXDemonPrimaryButton(text = stringResource(R.string.open_terminal), onClick = onOpenTerminal)
        Spacer(Modifier.height(YourXDemonTokens.Spacing.SM))
        HomeQuickActions(onBackup = onBackup, onStatus = onStatus)
        Spacer(Modifier.height(YourXDemonTokens.Spacing.SM))
        Row(horizontalArrangement = Arrangement.spacedBy(YourXDemonTokens.Spacing.SM)) {
            YourXDemonGhostButton(text = stringResource(R.string.restart), onClick = onRestart, modifier = Modifier.weight(1f))
            YourXDemonDestructiveButton(text = stringResource(R.string.stop), onClick = onStop, modifier = Modifier.weight(1f))
        }
    } else if (isStarting) {
        YourXDemonDestructiveButton(text = stringResource(R.string.stop), onClick = onStop)
    } else if (vmState is VmState.Error) {
        YourXDemonPrimaryButton(text = stringResource(R.string.try_again), onClick = onStart)
        Spacer(Modifier.height(YourXDemonTokens.Spacing.SM))
        HomeQuickActions(onBackup = onBackup, onStatus = onStatus)
    } else {
        YourXDemonPrimaryButton(text = stringResource(R.string.start_vm), onClick = onStart)
        Spacer(Modifier.height(YourXDemonTokens.Spacing.SM))
        HomeQuickActions(onBackup = onBackup, onStatus = onStatus)
    }
}
