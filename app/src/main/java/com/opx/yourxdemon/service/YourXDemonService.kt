 ...
 *
 * Foreground service that hosts the QEMU process for YourXDemon.
 * Holds a WakeLock to prevent the device from sleeping while the VM runs.
 */
package com.opx.yourxdemon.service
...
import com.opx.yourxdemon.MainActivity
import com.opx.yourxdemon.YourXDemonApplication
...
import com.opx.yourxdemon.engine.VmConfig
import com.opx.yourxdemon.engine.VmEngine
import com.opx.yourxdemon.engine.VmState
import com.opx.yourxdemon.engine.usb.UsbPassthroughManager
...
@AndroidEntryPoint
class YourXDemonService : Service() {
    ...
    @Inject lateinit var notificationPoster: com.opx.yourxdemon.engine.hostbridge.AndroidNotificationPoster
    @Inject lateinit var headlessModeManager: com.opx.yourxdemon.engine.hostbridge.HeadlessModeManager
    private var hostRequestServer: com.opx.yourxdemon.engine.hostbridge.HostRequestServer? = null
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                // launchYourXDemon relaunches the observers (resetting their
    ...
    private suspend fun observeStateForShutdown() {
        ...
        // launchYourXDemon is about to run flips it to Starting. Acting on that
    ...
    private fun ensureHostBridge(): com.opx.yourxdemon.engine.hostbridge.HostRequestServer {
        hostRequestServer?.let { return it }
        val dispatcher = com.opx.yourxdemon.engine.hostbridge.HostRequestDispatcher(
        ...
        return com.opx.yourxdemon.engine.hostbridge.HostRequestServer(
        ...
        serviceScope.launch {
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    (application as? YourXDemonApplication)?.awaitAssetsReady()
                    ...
                    if (sshEnabled && rules.none { it.hostPort == SSH_HOST_PORT }) {
                        rules.add(com.opx.yourxdemon.data.repository.PortForwardRule(SSH_HOST_PORT, 22, "tcp"))
                    }
                    ...
                    if (rules.none { it.hostPort == X11Constants.VNC_PORT }) {
                        rules.add(com.opx.yourxdemon.data.repository.PortForwardRule(X11Constants.VNC_PORT, X11Constants.VNC_PORT, "tcp", loopbackOnly = true))
                    }
                    if (rules.none { it.hostPort == X11Constants.AUDIO_PORT }) {
                        rules.add(com.opx.yourxdemon.data.repository.PortForwardRule(X11Constants.AUDIO_PORT, X11Constants.AUDIO_PORT, "tcp", loopbackOnly = true))
                    }
                    ...
                    val config = VmConfig(
                        ...
                        androidIp = NetworkUtils.localIpv4(this@YourXDemonService),
                        dnsServers = NetworkUtils.dnsServers(this@YourXDemonService),
    ...
    private fun getOrCreateNotificationBuilder(): NotificationCompat.Builder {
        ...
        val stopIntent = PendingIntent.getService(
            this, 1,
            Intent(this, YourXDemonService::class.java).apply { action = ACTION_STOP },
            PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT,
        )
    ...
    private fun handleOpenUrl(url: String): String {
        ...
        if (uri == null || uri.scheme?.lowercase() !in setOf("http", "https")) {
            return com.opx.yourxdemon.engine.hostbridge.HostProtocol.err("only http/https URLs are allowed")
        }
        return try {
            val intent = Intent(Intent.ACTION_VIEW, uri).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            applicationContext.startActivity(intent)
            com.opx.yourxdemon.engine.hostbridge.HostProtocol.ok()
        } catch (e: Throwable) {
            com.opx.yourxdemon.engine.hostbridge.HostProtocol.err("no app available to open this URL")
        }
    }

    private fun handleHeadlessRequest(action: String): String = when (action) {
        "on" -> { headlessModeManager.setActive(true); com.opx.yourxdemon.engine.hostbridge.HostProtocol.ok() }
        "off" -> { headlessModeManager.setActive(false); com.opx.yourxdemon.engine.hostbridge.HostProtocol.ok() }
        "status" -> com.opx.yourxdemon.engine.hostbridge.HostProtocol.ok(if (headlessModeManager.active.value) "on" else "off")
        else -> com.opx.yourxdemon.engine.hostbridge.HostProtocol.err("usage: on|off|status")
    }
    ...
    private fun handlePowerRequest(action: String): String {
        val proto = com.opx.yourxdemon.engine.hostbridge.HostProtocol
        return when (action) {
            ...
            "stop" -> {
                ...
                android.os.Handler(android.os.Looper.getMainLooper())
                    .postDelayed({ YourXDemonService.stop(ctx) }, 300)
                proto.ok()
            }
    ...
    companion object {
        ...
        const val ACTION_STOP    = "com.opx.yourxdemon.action.STOP"
        const val SSH_HOST_PORT  = 9922

        fun start(context: Context) {
            val intent = Intent(context, YourXDemonService::class.java).apply {
                action = ACTION_START
            }
        ...
        fun stop(context: Context) {
            context.startService(Intent(context, YourXDemonService::class.java).apply {
                action = ACTION_STOP
            })
        }
    }
 ...
 * [YourXDemonService]'s own companion, so there's nothing privileged to gate. Kept
...
class VmControlReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        if (settingsRepository.getAutostartOnBootSnapshot()) {
                            dispatch { YourXDemonService.start(context.applicationContext) }
                        }
                    } finally {
                        pending.finish()
                    }
            ...
            ACTION_START_VM -> dispatch { YourXDemonService.start(context.applicationContext) }
            ACTION_STOP_VM  -> dispatch { YourXDemonService.stop(context.applicationContext) }
        }
    }
     ...
     * warning naming the cure (launch YourXDemon itself so its activity's
        ...
        try {
            ...
            Log.w(
                ...
                "VmControlReceiver could not start/stop the VM from the background " +
                    ...
                    "launch YourXDemon so its START_VM activity intent runs in the foreground, " +
    ...
    companion object {
        ...
        const val ACTION_START_VM = "com.opx.yourxdemon.action.START_VM"
        const val ACTION_STOP_VM  = "com.opx.yourxdemon.action.STOP_VM"
    }
}
