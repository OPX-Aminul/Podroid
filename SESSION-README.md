...
**Date:** August 2026
**Branch:** `main`
**Original developer:** ExTV (OPX)
**Rebranded by:** OP Aminul FF (OPX)
**Summary:** Full rebranding from OPX → YourXDemon, USB WiFi fix, WiFi firmware coverage, rootfs size reduction, CI/CD overhaul, release build optimization, TCP Guest Agent latency optimization
...
Release APK is signed with the project's release keystore (`opx-release.jks`) configured in `build.gradle.kts` `signingConfigs.release`.
...
| `build-rootfs/files/etc/init.d/yourxdemon-agentd` | OpenRC service script — auto-starts `yourxdemon-agentd` after `opx-bootstrap` in the default runlevel. |
...
| `Dockerfile` | Rebranded `init-opx` → `init-yourxdemon` references. |
...
| `236d187` | Full A-to-Z rebranding OPX → YourXDemon |
| `e83ebc1` | Rebranding OPX to YourXDemon (initial pass) |
...
| `initrd.img` | 40 MB |
| `libslirp.so` | 3 MB |
| `libopx-bridge.so` | <1 MB |
| `libopx-launcher.so` | <1 MB |
...
## StrykerApp Build Script (Pivot Ready)

Build script created at `stryker-build/` for StrykerApp QEMU with Xiaomi USB fix:
- `stryker-build/Dockerfile` — Full QEMU build with all patches (Xiaomi, PAC, LIBUSB)
- `stryker-build/README.md` — Comprehensive guide for StrykerApp integration
...
### Deep Analysis (Aug 30, 2026)

Complete source code + binary analysis performed:

**Stock Binary (rootless-main release):**
- QEMU 11.0.2 (vanilla source paths confirmed)
- All USB features present (usb-host, usb-xhci, wrap_sys_device, SCM_RIGHTS)
- Binary: 42MB (our build: ~52MB — different compilation)
- libslirp.so: 1.1MB (our build: 3MB)

**App Source Code (full review):**
- QEMU command construction in RootlessEngine.java
- USB attach: UsbPassthroughManager.java → QmpClient.addFd → device_add
- Guest agent: socat-based (572B shell script)
- Ports: 1050 (exec), 1051 (PTY), 2222 (SSH)

**Critical Finding:**
Our Dockerfile adds 5 patches. The **PAC coroutine fix** (replaces sigsetjmp with custom assembly) is the highest-risk — it may break libusb's internal error handling, causing USB operations to fail silently.

**Next Steps:**
1. Build QEMU with ZERO extra patches → test USB
2. Add patches one by one to find the culprit
3. Binary diff between stock and our build

**Full analysis:** `stryker-build/STOCK-BINARY-ANALYSIS.md`

---

*Generated by Buffy (Codebuff) — YourXDemon development session, August 2026*
*Original project: OPX by ExTV | Rebranded by OP Aminul FF (OPX)*
