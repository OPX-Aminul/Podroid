# ─────────────────────────────────────────────────────────────────────────────
# YourXDemon Unified Dockerfile
# Combines Custom Kernel, Initramfs (Debian Trixie VM) and QEMU (Android ARM64) builds.
# ─────────────────────────────────────────────────────────────────────────────
...
# Xiaomi/MIUI USB speed correction — guest kernel patch (issue #85 backup).
...
# ==============================================================================
# SECTION 1: Initramfs (Debian Trixie VM) Build
# ==============================================================================

# Stage 1: Minimal boot environment with busybox and core utils
FROM --platform=linux/arm64/v8 debian:trixie AS rootfs-builder
RUN apt-get update && apt-get install -y --no-install-recommends \
    busybox-static \
    bash \
    kmod \
...
RUN chmod +x /init
RUN rm -rf /var/cache/apt/archives/* /tmp/* /var/tmp/* /usr/share/man /usr/share/doc

# Stage 2: Pack Initramfs
FROM debian:trixie AS packer
RUN apt-get update && apt-get install -y --no-install-recommends cpio gzip findutils \
    && rm -rf /var/lib/apt/lists/*
